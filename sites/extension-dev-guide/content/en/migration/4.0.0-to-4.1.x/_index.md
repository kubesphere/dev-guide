...
##### Use the Private npm Registry

All `@ks-console/*` dependencies have been migrated to our self-hosted npm registry `https://registry.npm.kubesphere.com.cn`. If you need the account name and password for this registry, contact us at <kubesphere@yunify.com>. Packages published on `https://www.npmjs.com` will be removed after the documentation is updated, so do not install these dependencies from npmjs.org anymore.

1. Create or update the `.npmrc` file in the root directory of your extension project (projects generated by `yarn create ks-project` already contain this file) with the following configuration:

   ```ini
   @ks-console:registry=https://registry.npm.kubesphere.com.cn
   ```

2. Log in to the registry and run the following commands to make sure the connection and credentials work:
...
{{% notice note %}}
Send an email to <kubesphere@yunify.com> if you need to reset your credentials or request access for new team members.
{{% /notice %}}
