...
在这一背景下，我们希望减少隔离的需求，实现更轻量级的"微前端"，或者我们可以称之为`微模块`。在微模块架构中，子应用和主应用共享相同的技术栈，并能够共享运行时。这有助于实现更一致的用户体验、更高的集成度和更便捷的依赖共享，从而提高运行效率。正如上面的架构图所示，扩展组件的开发依赖于通用的 `KubeDesign`、`@ks-console/shared` 等库。然后，可以使用脚手架、CLI 等工具进行打包和发布。在 Core（基座）的部分实现了扩展组件的注册和运行。
